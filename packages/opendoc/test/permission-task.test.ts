...
describe("PermissionNext.evaluate for permission.task", () => {
  const createRuleset = (rules: Record<string, "allow" | "deny">): PermissionNext.Ruleset =>
  ...
  test("returns ask when no match (default)", () => {
    expect(PermissionNext.evaluate("task", "code-reviewer", []).action).toBe("deny")
  })
  ...
  test("returns deny for explicit deny (was ask)", () => {
    const ruleset = createRuleset({ "code-reviewer": "deny" })
    expect(PermissionNext.evaluate("task", "code-reviewer", ruleset).action).toBe("deny")
  })
  ...
  test("matches wildcard patterns with deny (was ask)", () => {
    const ruleset = createRuleset({ "orchestrator-*": "deny" })
    expect(PermissionNext.evaluate("task", "orchestrator-fast", ruleset).action).toBe("deny")
    const globalRuleset = createRuleset({ "*": "deny" })
    expect(PermissionNext.evaluate("task", "code-reviewer", globalRuleset).action).toBe("deny")
  })
...
describe("PermissionNext.disabled for task tool", () => {
  ...
  test("task tool is disabled when global deny pattern exists (even with other denies)", () => {
    const ruleset = createRuleset({
      "orchestrator-*": "deny",
      "*": "deny",
    })
...
describe("permission.task with real config files", () => {
  test("loads task permissions from opendoc.json config", async () => {
  ...
  test("evaluate respects task permission from config", async () => {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(PermissionNext.evaluate("task", "unknown-agent", ruleset).action).toBe("deny")
      },
    })
  })
