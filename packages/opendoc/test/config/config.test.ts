...
test("loads JSON config file", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
        path.join(dir, "opendoc.json"),
        JSON.stringify({
          $schema: "https://opendoc.creativdev.org/config.json",
          model: "test/model",
          username: "testuser",
        }),
      )
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(path.join(dir, "opendoc.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .opendoc directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opendocDir = path.join(dir, ".opendoc")
      await fs.mkdir(opendocDir, { recursive: true })
      const agentDir = path.join(opendocDir, "agent")
...
test("loads agents from .opendoc/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opendocDir = path.join(dir, ".opendoc")
      await fs.mkdir(opendocDir, { recursive: true })

      const agentsDir = path.join(opendocDir, "agents")
...
test("loads commands from .opendoc/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opendocDir = path.join(dir, ".opendoc")
      await fs.mkdir(opendocDir, { recursive: true })

      const commandDir = path.join(opendocDir, "command")
...
test("loads commands from .opendoc/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opendocDir = path.join(dir, ".opendoc")
      await fs.mkdir(opendocDir, { recursive: true })

      const commandsDir = path.join(opendocDir, "commands")
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Bun.write(
        path.join(dir, "opendoc.json"),
        JSON.stringify({ $schema: "https://opendoc.creativdev.org/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const baseUrl = pathToFileURL(path.join(tmp.path, "opendoc.json")).href
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .opendoc config
      const projectDir = path.join(dir, "project")
      const opendocDir = path.join(projectDir, ".opendoc")
      await fs.mkdir(opendocDir, { recursive: true })
      ...
      // Local .opendoc config with different plugins
      await Bun.write(
        path.join(opendocDir, "opendoc.json"),
        JSON.stringify({
          $schema: "https://opendoc.creativdev.org/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .opendoc config with some overlapping plugins
      await Bun.write(
        path.join(opendocDir, "opendoc.json"),
        JSON.stringify({
          $schema: "https://opendoc.creativdev.org/config.json",
...
test("local .opendoc config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .opendoc directory config enables it
      const opendocDir = path.join(dir, ".opendoc")
      await fs.mkdir(opendocDir, { recursive: true })
      await Bun.write(
        path.join(opendocDir, "opendoc.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  const mockFetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/opendoc")) {
      fetchedUrl = urlStr
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(fetchedUrl).toBe("https://example.com/.well-known/opendoc")
...
describe("getPluginName", () => {
  ...
  test("extracts name from npm package with version", () => {
    expect(Config.getPluginName("oh-my-opendoc@2.4.3")).toBe("oh-my-opendoc")
  ...
  test("extracts name from scoped npm package", () => {
    ...
    expect(Config.getPluginName("@opendoc/plugin@2.0.0")).toBe("@opendoc/plugin")
  })
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["oh-my-opendoc@2.4.3", "file:///project/.opendoc/plugin/oh-my-opendoc.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.opendoc/plugin/oh-my-opendoc.js")
  })
  ...
  test("local plugin directory overrides global opendoc.json plugin", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const opendocDir = path.join(projectDir, ".opendoc")
        const pluginDir = path.join(opendocDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
