...
export namespace PermissionNext {
  ...
  // Removed "ask" - this fork is for non-interactive API mode (documentation assistant)
  // Only "allow" and "deny" are supported
  export const Action = z.enum(["allow", "deny"]).meta({
  ...
  export const ask = fn(
    ...
    async (input) => {
      ...
      for (const pattern of request.patterns ?? []) {
        ...
        // Only "allow" and "deny" are supported, no blocking
  ...
  export function evaluate(permission: string, pattern: string, ...rulesets: Ruleset[]): Rule {
    ...
    // Changed from "ask" to "deny" - no matching rule means deny by default
    return match ?? { action: "deny", permission, pattern: "*" }
  }
