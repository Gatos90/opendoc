...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run opendoc",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.exitCode === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for opendoc session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found opendoc session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await $`opendoc import ${sessionUrl}`.nothrow()
        ...
        UI.println()
        UI.println("Starting opendoc...")
        UI.println()

        // Launch opendoc TUI with session ID if available
        const { spawn } = await import("child_process")
        const opendocArgs = sessionId ? ["-s", sessionId] : []
        const opendocProcess = spawn("opendoc", opendocArgs, {
          stdio: "inherit",
          cwd: process.cwd(),
        })

        await new Promise<void>((resolve, reject) => {
          opendocProcess.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`opendoc exited with code ${code}`))
          })
          opendocProcess.on("error", reject)
        })
      },
    })
  },
