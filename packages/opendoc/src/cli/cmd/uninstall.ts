...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall opendoc and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g opendoc-ai",
      pnpm: "pnpm uninstall -g opendoc-ai",
      bun: "bun remove -g opendoc-ai",
      yarn: "yarn global remove opendoc-ai",
      brew: "brew uninstall opendoc",
    }
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    const content = await Bun.file(file)
      .text()
      .catch(() => "")
    if (content.includes("# opendoc") || content.includes(".opendoc/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".opendoc/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".opendoc/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".opendoc"))
    ) {
      continue
    }
