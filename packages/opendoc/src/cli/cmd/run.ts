...
import { createOpencodeClient, type OpencodeClient } from "@opendoc-ai/sdk/v2"
...
export const RunCommand = cmd({
  command: "run [message..]",
  describe: "run opendoc with a message",
  builder: (yargs: Argv) => {
    return yargs
      .positional("message", {
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running opendoc server (e.g., http://localhost:4096)",
      })
      .option("port", {
        type: "number",
  ...
  handler: async (args) => {
    ...
    if (args.attach) {
      ...
      if (cfgResult.data && (cfgResult.data.share === "auto" || Flag.OPENDOC_AUTO_SHARE || args.share)) {
    ...
    await bootstrap(process.cwd(), async () => {
      ...
      const sdk = createOpencodeClient({ baseUrl: "http://opendoc.internal", fetch: fetchFn })
