...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for opendoc.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal opendoc sessions
...
opendoc acp --cwd /path/to/project
```
...
- **Terminal Integration**: Full terminal support via opendoc's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | opendoc acp
```
...
ACP sessions map cleanly to opendoc's internal session model:
