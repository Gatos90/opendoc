...
import type { OpencodeClient, SessionMessageResponse } from "@opendoc-ai/sdk/v2"
...
export namespace ACP {
  ...
  export class Agent implements ACPAgent {
    ...
    async initialize(params: InitializeRequest): Promise<InitializeResponse> {
      ...
      const authMethod: AuthMethod = {
        description: "Run `opendoc auth login` in the terminal",
        name: "Login with opendoc",
        id: "opendoc-login",
      }
  ...
  async function defaultModel(config: ACPConfig, cwd?: string) {
    ...
    const opendocProvider = providers.find((p) => p.id === "opendoc")
    if (opendocProvider) {
      if (opendocProvider.models["big-pickle"]) {
        return { providerID: "opendoc", modelID: "big-pickle" }
      }
      const [best] = Provider.sort(Object.values(opendocProvider.models))
      if (best) {
        return {
